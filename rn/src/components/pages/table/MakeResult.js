...
class MakeResult extends Component {
  ...
  getStage0 = () => {
    return (
      <View>
        ...
        <GamePicker value={this.state.game} onSelectGame={this.setGame} />
        {this.state.game && (
          <Button text="Continue" onPress={this.goToStage1} />
        )}
      </View>
    )
  }
    ...
    return (
      <View>
        ...
        {this.state.players.length > 1 && (
          <Button text="Continue" onPress={this.goToStage2} />
        )}
      </View>
    )
  }
    ...
    return (
      <View>
        ...
        {this.state.winners.length > 1 &&
          this.state.winners.length < this.state.players.length && (
            <Button text="Submit" onPress={this.createResult} />
          )}
      </View>
    )
  }
  ...
  getPlayerChecklist = () => {
    ...
    this.state.tablePlayers.forEach(player => {
      let gameElo = player.elos.find(
        elo => elo.game._id === this.state.game._id
      )
      return (
        <View key={player._id}>
          <Checky
  ...
  getWinnerChecklist = () => {
    ...
    this.state.players.forEach(player => {
      let gameElo = this.state.tablePlayers.elos.find(
        elo => elo.game._id === this.state.game._id
      )
      return (
        <View key={player}>
