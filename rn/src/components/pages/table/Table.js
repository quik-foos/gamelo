...
class Table extends Component {
  ...
  cancelJoinRequest = async () => {
    try {
      await TableApi.removeJoinRequest({
        id: this.props.id,
  ...
  getWaitingView = () => {
    return (
      <View>
        ...
        <Button text="Cancel request" onPress={this.cancelJoinRequest} />
      </View>
    )
  }
  ...
  getHostView = () => {
    return (
      <View>
        ...
        {this.getCurrentPlayers()}
        {this.state.status === 'Scheduled' && (
          <Button text="Start playing" onPress={this.startPlaying} />
        )}
        {(this.state.status === 'In-Progress' ||
          this.state.status === 'Completed') &&
          this.getResults()}
        {this.state.status === 'In-Progress' && (
          <Button text="End session" onPress={this.endSession} />
        )}
        {this.state.status === 'In-Progress' || (
          <Button
            text="Record game result"
        ...
        <Button text="Cancel Table" onPress={this.cancelTable} />
      </View>
    )
  }
  ...
  getCreateResultView = () => {
    return (
      <MakeResult cancelAction={this.goToNormalView} tableId={this.props.id} />
    )
  }
  ...
  getCurrentPlayers = () => (
    <Fragment>
      <Text>
        Current Players {this.state.players.length} / {this.state.max}{' '}
      </Text>
      {this.state.players.map((player, key) => {
        return (
          <TouchableOpacity
            key={key}
            onPress={() => {
              this.navigateToProfile(player._id)
            }}
          >
            <View>
        ...
        return (
          <View key={id}>
            ...
            <ButtonSmall
              text="Accept"
              onPress={() => {
                this.acceptJoinRequest(joinRequest._id)
              }}
            />
            <ButtonSmall
              text="Reject"
              onPress={() => {
                this.rejectJoinRequest(joinRequest._id)
              }}
            />
          </View>
        )
      ...
      {this.state.results.map((result, id) => {
        return (
          <TouchableOpacity
            key={id}
            onPress={() => {
              this.goToResultView(result._id)
            }}
          >
            <Text>{result.game.name}</Text>
          </TouchableOpacity>
        )
      })}
    </Fragment>
  ...
  render() {
    ...
    if (
      this.state.players.map(player => player._id).includes(this.props.user)
    ) {
      // This is a guest
