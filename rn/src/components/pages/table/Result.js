...
class Result extends Component {
  ...
  getPlayers = () => {
    return (
      <View>
        {this.state.result.players.map(player => (
          <View>
            <Text>{player._id}</Text>
            {this.state.result.winner
              .map(winne => winne._id)
              .includes(player._id) && <Text>Winner</Text>}
          </View>
        ))}
        ;
      </View>
  ...
  getButtons = () => {
    let validated = false
    if (
      this.state.result.validatedPlayers
    ...
    return (
      <View>
        <Button text="Return" onPress={this.props.cancelAction} />
        <Button text="Accept" onPress={this.acceptResult} />
      </View>
    )
  }
